...
### Обработка падения микросервисов

Система реализует обработку падения сервиса аналитики - при отказе система продолжает работать, выполняя сохранение сданных работ.

**Сценарий 1: Падение File Analysis Service**

Если Analysis Service недоступен:
- File Storing Service **успешно сохраняет файл** в базу данных и на диск
- В логах фиксируется предупреждение: *"Не удалось запустить анализ для submission {id}"*
- Пользователь получает успешный ответ с данными submission
- Анализ может быть выполнен позже при восстановлении сервиса
